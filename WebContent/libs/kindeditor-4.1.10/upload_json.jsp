...
String savePath = appPath.replaceAll(appName, "ROOT") + "/qgjFiles/QRCodeImgs/";
...
//文件保存目录URL
String saveUrl = "/qgjFiles/QRCodeImgs/";
...
extMap.put("flash", "swf,flv");
extMap.put("media", "mp3,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb,mp4");
extMap.put("file", "gif,jpg,jpeg,png,bmp,doc,docx,xls,xlsx,ppt,htm,html,txt,zip,rar,gz,bz2");
...
while (itr.hasNext()) {
	...
	if (!item.isFormField()) {
		...
		if(item.getFieldName().equals("imgFile")&&item.getSize()>102400){
			out.println(getError("图片不要超过100K。"));
			return;
		}
		//检查扩展名
