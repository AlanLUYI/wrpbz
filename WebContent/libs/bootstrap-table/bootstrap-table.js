...
!function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        if(this.options.numbers){
            html.push(sprintf("<th style='text-align: %s; width: %spx;'>", "center", 36));
            html.push("<div class='ht-inner'></div>");
        ...
        this.$header.find('th').each(function (i) {
            if(that.options.numbers){
                if(i == 0){
                    $(this).data({sortable: false});
                }else{
                    $(this).data(visibleColumns[i-1]);
                }
            }else{
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            if(this.options.numbers){
                html.push(sprintf('<td style="padding: 8px 0; text-align: center;">%s</td>', i+1));
            }
        ...
        if (!html.length) {
            html.push('<tr class="no-records-found">',
                sprintf('<td colspan="%s">%s</td>', this.options.numbers ? this.header.fields.length + 1 : this.header.fields.length, this.options.formatNoMatches()),
                '</tr>');
        }
        ...
        $.each(this.header.events, function (i, events) {
            ...
            for (var key in events) {
                that.$body.find('tr').each(function () {
                    var $tr = $(this),
                        tdIndex = that.options.numbers ? i +1 : i,
                        $td = $tr.find(that.options.cardView ? '.card-view' : 'td').eq(tdIndex),
                        index = key.indexOf(' '),
